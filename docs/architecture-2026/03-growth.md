...
## Gemini Review Findings, round 2 — P3 build review (2026-07-30, later same day)

A second review pass, run against the actual `compute-lead-scores.py` code once it shipped: **finding #2 above (recency should multiplicatively decay the total, not add up to 10 points) was correctly identified in the original review but got missed in the first build** — the shipped script initially summed all five components additively, including recency, reproducing the exact flaw the original review called out. Caught by re-review, not by new production data. **Fixed same day:** `recency_score()` was replaced with `recency_decay()`, returning a 0-1 factor; the final score is now `round((intent + pipeline_depth + engagement + territory) * decay)`. The `lead_scores.recency_score` column is kept but now stores the decay factor as a 0-100 percentage (informational — "how stale is this lead") rather than a points contribution, so the schema didn't need to change. Re-run live against the same 5 real contacts after the fix — no errors, scores now correctly discount stale leads.

...
## Gemini Review Findings, round 2 — P3/P4 build review (2026-07-30, later same day)

A second review pass was run against the actual P3/P4 code once it shipped, not just the design. Two real findings, both fixed same-day, before any live Team-tier account existed:

1. **`org_id = the owner's own firebase_uid` (as shipped) breaks on owner account deletion, ownership transfer, and tier downgrade.** Coupling an organization's identity to one user account means: deleting the owner's account orphans every `org_members` row under it (headless org, no one can manage the roster or be billed); transferring ownership requires rewriting every member row's `org_id` instead of updating one row; downgrading from Team to Pro leaves member rows pointing at an owner whose tier no longer supports them, with no defined behavior. **Fix applied same day:** migration 040 adds a real `organizations` table (`id`, `owner_uid`, `name`) and `org_members.org_id` becomes a proper `BIGINT REFERENCES organizations(id)` instead of a bare `firebase_uid` string. Cheap to fix immediately rather than as a future migration — `org_members` had zero live rows when this was caught.
2. **Deactivating a user only revoked their Firebase session, not their API keys.** `POST /v1/ops/customer/{uid}/deactivate` set `is_active = false` and called `revokeRefreshTokens`, but `require_api_key` (the separate auth path API keys use, migration 031) never checks `user_profiles.is_active` at all — a deactivated user's existing API key kept working. **Fix applied:** `deactivate_customer` now also sets `revoked_at = now()` on every live `api_keys` row for that user; `require_api_key` already checked `revoked_at IS NULL`, so this closes the gap without any new schema or dependency change.

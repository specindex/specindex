...
```bash
chmod +x scripts/nj_dca_pipeline/run_job.sh
```

3. Manual smoke test:

```bash
./scripts/nj_dca_pipeline/run_job.sh
# or:
...
```cron
0 2 * * * /bin/bash /path/to/specindex/scripts/nj_dca_pipeline/run_job.sh
```
...
```cron
0 2 * * * /bin/bash /Users/ahussain2373/Projects/specindex/scripts/nj_dca_pipeline/run_job.sh >> /Users/ahussain2373/Projects/specindex/data/pipeline/nj-dca/cron.out 2>&1
```
